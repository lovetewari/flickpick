...
// Collect flatrate providers (name + official logo url), deduped by name.
function collectProv(region) {
  const seen = new Map();
  for (const p of region.flatrate || []) {
    const name = PROV[p.provider_id] || p.provider_name;
...
async function enrichMovie(raw) {
  ...
  return {
    ...
    genres: (raw.genre_ids || []).map(g => GENRE[g]).filter(Boolean),
...
async function upsert(rows) {
  for (let i = 0; i < rows.length; i += 100) {
    ...
    const { error } = await sb.from('catalog').upsert(batch, { onConflict: 'id' });
